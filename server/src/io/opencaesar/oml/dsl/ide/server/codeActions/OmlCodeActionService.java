	...
	public List<Either<Command, CodeAction>> getCodeActions(ICodeActionService2.Options options) {
		if (options.getResource() != null) {
			...
			if (root instanceof Vocabulary)
				return createCodeActions((Vocabulary)root, options.getCodeActionParams(), options.getDocument());
		}
		return Collections.emptyList();
	}

	private List<Either<Command, CodeAction>> createCodeActions(Vocabulary vocabulary, CodeActionParams params, Document document) {
